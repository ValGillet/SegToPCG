...
def edges_to_graphene_blockwise(
                      fragments_file,
                      cloudpath,
                      db_host,
                      db_name,
    ...
    daisy.run_blockwise(
                ...
                process_function = lambda: edges_to_protobuf(
                                                    db_host,
                                                    db_name,
                                                    edges_collection,
                                                    total_roi,
    ...
    doc_ingest = {
              'cloudpath': cloudpath,
              'task': 'edges_translation',
              'frag_file': fragments_file,
              'edges_cloud': edges_dir_cloud,
...
def translate_edges_worker(db_host,
    ...
    while True:
        ...
        extended_roi = roi.grow(daisy.Coordinate(chunk_size)/2, 
                                daisy.Coordinate(chunk_size)/2) # Supercube including half of adjacent chunks
        ...
        main_chunk_id = get_chunkId(bit_per_chunk_dim, 
